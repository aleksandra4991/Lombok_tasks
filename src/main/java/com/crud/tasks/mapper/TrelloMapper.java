...
public class TrelloMapper {
    ...
    public List<TrelloList> mapToList(final List<TrelloListDto2> trelloListDto2) {
        return trelloListDto2.stream()
                .map(trelloList -> new TrelloList(trelloList.getId(), trelloList.getName(), trelloList.isClosed()))
                .collect(toList());
    }

    public List<TrelloListDto2> mapToListDto(final List<TrelloList> trelloLists) {
        return trelloLists.stream()
                .map(trelloList -> new TrelloListDto2(trelloList.getId(), trelloList.getName(), trelloList.isClosed()))
                .collect(toList());
    }
